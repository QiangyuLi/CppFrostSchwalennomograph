 ...
 * Single file implementation using only standard library
 * No custom namespaces - ready for fast integration
 */
...
double interpolate(const std::vector<std::pair<double, double>>& table, double x) {
...
// Main evaporation loss calculation function
double calculateEvaporationLoss(double vpd, int nozzle, double pressure, double wind) {
    ...
    // Interpolate Y coordinates for each parameter
...
// Example usage
int main() {
    std::cout << "=== Minimal Evaporation Loss Calculator ===" << std::endl;
    std::cout << "Usage: calculateEvaporationLoss(vpd, nozzle, pressure, wind)" << std::endl;
    std::cout << "- vpd: Vapor-Pressure Deficit (psi)" << std::endl;
    std::cout << "- nozzle: Nozzle diameter (64ths inch)" << std::endl;
    std::cout << "- pressure: Pressure (psi)" << std::endl;
    std::cout << "- wind: Wind velocity (mph)" << std::endl;
    std::cout << std::endl;
    ...
    TestCase testCases[] = {
        {0.6, 12, 40, 5, "Standard conditions"},
    ...
    for (const auto& test : testCases) {
        double result = calculateEvaporationLoss(test.vpd, test.nozzle, test.pressure, test.wind);
